...
def run_p(main_addr, partitions, queue_in: Queue, queue_out: Queue, node_id: int = 0, stage: int = 0, seq_l: int = 256, n_layers = 4, 
                    batch_size = 8, dmodel = 256, num_heads = 16, memory = 3, mb_count = 12, cost_map = [],
                    device = "cuda"):
    manual_seed(0)
    world_size = 0
    for v in partitions:
    ...
    if stage == 0:
        ...
        with open(f'log{node_id}.txt', 'a') as file, redirect_stdout(file):
            loc =  SubP(queue_in,queue_out,net,optimizer,node_id,stage,ts,device=device, mb_count=mb_count, memory = memory)
            loc.start()
    else:
        ...
        loc =  SubP(queue_in,queue_out,net,optimizer,node_id,stage,None,device=device,  mb_count=mb_count, memory = memory)
        loc.start()
